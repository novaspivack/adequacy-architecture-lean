...
\title{\textbf{The Architecture of Outer Admissibility}\\[6pt]
  \large Why Outer Certificate Worlds Collapse, Layer, and Residualize}
...
Almost every formal or applied system that reasons about complex internal
structure must at some point operate through an external or interface-level
predicate --- a certificate that claims to stand in for richer content
without fully exposing it.  This is not a pathology; it is ubiquitous.

\begin{itemize}
  \item A \emph{compressed witness} claims to capture the essential content
    of a much richer proof or derivation.
  \item An \emph{abstraction layer} claims to preserve enough structure for
    downstream reasoning to proceed faithfully.
  \item A \emph{quotient or interface representation} projects internal
    distinctions into a coarser type and claims the projection is sufficient.
  \item A \emph{transport claim} asserts that a structure moved along a map
    or reindexing retains its certificate content.
  \item An \emph{upgrade witness} presents a certified object at one level
    as a certificate of a richer level.
\end{itemize}

In every case the outer predicate makes the same kind of claim: it says
``the relevant content is here, at this interface, in this form.''  Without
a theory of outer admissibility, such certificates are under-classified
and often over-trusted.  A pullback display, a branch-native pack, and
an upgrade witness look like three different kinds of outer certificate.
The central discovery of this paper is that \emph{they are not}: they are
forced presentations of one outer gate, and richer content is forced
upward into higher layers rather than sideways into new primitive modes.

That is the architectural fact this paper proves.
...
\subsection{Why this is an architectural fact, not a classification exercise}

The six points above could sound like a tidy taxonomy.  They are not.
They are a structural impossibility result.

The core message is this: \emph{you cannot define a richer outer
admissibility condition by composing or lifting from the normal forms and
exceed the outer gate.}  The grammar-cut maximality theorem says so
directly.  And you cannot reach the second-layer content by any
admissibility route --- it is produced canonically \emph{after} admission,
not as an alternative to it.  And you cannot absorb the branch-specific
residual content into either layer --- it has a genuine irreducible piece
that the outer gate alone cannot supply.

This means: the three levels are not three choices.  They are three
different kinds of structural content, each forced into its own band by
the logic of admissibility itself.
