...
This paper answers all three questions.  The answers are not three separate
results.  They are three facets of the same architectural fact:
\emph{outer certificate worlds do not define multiple admissibility worlds;
they are forced into one layered architecture with a unique outer gate,
a derived second layer above it, and a genuine residual band beyond.}
...
\lean{highestReachable\-\_summit\_at}
(\leanmod{Lawful/FinalConditional\-Summit.lean}) packages the highest
reachable summit at any carrier as a structured machine-checked object
$H_{\mathrm{final}}$.  This is the target all outer routes aim to supply.
The unification theorem (\S\ref{sec:gate}) shows that all three constructor
arms of $\Gadm$ supply this target through $\Upb$: the three routes to the
summit are logically equivalent.

\subsection{Residual geometry}

The Lean package imports a residual-geometry library formalizing what
forgetful morphisms lose.  Every forgetful morphism has a canonical
fundamental residual structure: a machine-checked record of what was lost
in the map, what still observes it, and what the geometry of the loss
looks like.  This is the machine-checked form of the diagnostic principle
``if the burden is invisible, suspect the map,'' and it provides the
general framework in which the residual band of \S\ref{sec:residual} is
a specific instance.
(Lean name: \lean{every\-\_map\-\_has\-\_fundamental\-\_residual\-\_package},
\leanmod{Instances/WhatMapsForget.lean}.)
...
\lean{summitReportOf\-\_indexed\-\_master\-\_agreement\-\_consequences}
(\leanmod{Instances/NEMSSemantic\-FaithfulTransport.lean})
packages all consequences of master-indexed summit agreement as a single
bundle.  This is the branch-level analogue of the outer gate: the
halting/semantic faithfulness story collapses to a single master
agreement, just as the outer gate collapses to a single $\Upb$.
