...
``the relevant content is here, at this interface, in this form.''  The
deeper question --- almost never asked directly --- is whether these
different forms of outer certification are structurally the same thing, or
genuinely different things.  And if they are structurally the same, what
is the architecture above that sameness?

This paper is about those questions.
...
\begin{description}
  \item[\textbf{No free adequate collapse.}]
    ...
    all burden without paying elsewhere.
    (\lean{no\-\_free\-\_adequate\-\_collapse},
    \leanmod{Burden/NoFreeCollapse.lean})
  \item[\textbf{Burden relocates; it does not vanish.}]
    Honest simplification moves structural obligations elsewhere.
    (\lean{burden\_not\_erased\_only\_relocated},
...
This paper answers all three questions.  The answers constitute the
three-level architecture: \emph{collapse, layering, and residuality}.
...
The grammar-cut maximality theorem says: you cannot build a broader outer
class by composing from the normal forms.  The layering theorem says: you
cannot reach the second-layer content by any admissibility route --- it is
produced canonically \emph{after} admission.  The residuality result says:
you cannot absorb the branch-specific residual into the gate.

The practical consequence is diagnostic.  When someone presents an outer
predicate as an admissibility condition, the architecture asks four
questions that the predicate must answer:
\begin{itemize}
  \item Does it imply an absolute frontier certificate? (Gate consequence.)
  \item Is it grammar-generated?  (Maximality test.)
  \item Does it unlock a second layer, or is it itself second-layer content?
    (Layering test.)
  \item Is it primitive, or residual branch-specific content?  (Residuality
    test.)
\end{itemize}
Without answers to these questions, the admissibility claim is not fully
characterized.  With them, its position in the three-level architecture is
determined.
...
forgetful morphisms lose.  The canonical theorem \lean{every\-\_map\-\_has\-\_fundamental\-\_residual\-\_package}
...
Point IV says: the gate is not a dead end but the entrance to a forced
richer world.  Point V says: the richer branch-specific content is
accounted for but does not collapse the gate.

Together these five points characterize the full structure of outer
admissibility: \emph{collapse below, forcing above, residuality beyond.}
