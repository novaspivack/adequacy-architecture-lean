...
\textbf{The outer gate is not an accident; it is the unique ceiling.}
Grammar-cut maximality says: once the normal-form grammar is fixed, no
class generated from it can exceed the unified gate.  This turns
$\Gadm \equiv \Upb$ from ``one successful unification'' into ``the unique
admissibility ceiling of the declared certificate world.''  The gate is
not the highest thing we found --- it is the highest thing there is, within
the grammar.

\textbf{The second layer cannot be reached from below.}
No admissibility route --- not pullback display, not branch pack, not
upgrade witness --- produces the second-layer content directly.  It is
produced canonically \emph{after} admission, from the same witness, with
no new hypotheses.  It is what the gate forces into existence above itself.
This is why the two levels cannot be conflated.

\textbf{The residual content cannot be absorbed into either level.}
The branch-specific residual condition carries a genuine irreducible piece
that the outer gate alone cannot supply.  The residuality result locates
this content precisely and shows it cannot be pushed back down into the
gate or the second layer.

The practical consequence is diagnostic.  When someone presents an outer
predicate as an admissibility condition, the architecture asks:
\begin{itemize}
  \item Does it imply an absolute frontier certificate? (Gate consequence.)
  \item Is it grammar-generated?  (Ceiling test.)
  \item Does it unlock a second layer, or is it itself second-layer
    content?  (Layering test.)
  \item Is it a primitive outer mode, or residual branch-specific content?
    (Residuality test.)
\end{itemize}
Without answers, the claim is not fully characterized.  With them, its
position in the forced three-level architecture is determined.
