/-
  ...
  **Concrete bridge (Working queue item 1):** **`Instances/NemsSummitCarrierBridge`** — **`NemsSummitCarrierBridgeData`**, **`nemsSummitCarrierBridgeData_prodGraph`** (graph / product **`(B_γ, i, σ)`**), **`nemsFe3IndexedPhantomOps_carrier_bridge_induces_fe3`**; note **`SPEC_032_CB1`**. **Tagged staging:** **`Instances/TaggedHaltingSummitMasterStaging`** — **`nemsSummitCarrierBridgeData_taggedGraph`**, **`nemsFe3IndexedPhantomOps_carrier_taggedGraph_eq_pullbackAlongDom_b`**, **`SPEC_032_HM6_QT3`** retag / **`summitReportOf`** blindness; **`SPEC_032_HM7_VW2`** **`taggedStaging_tagWitness_branchGeneric_*`** (branch-generic tag-live, not NEMS); note **`SPEC_032_HM5_RK2`**.
  **Compare triangle (Working queue item 2):** **`NemsSummitCarrierBridgeCompareAlignment`**, **`nemsFe3IndexedPhantomOps_pullbackAlongDom_align_factors_through_compare_then_host`**; note **`SPEC_032_PI1`** (**tagged graph:** **`TaggedHaltingSummitMasterStaging`** — **`nemsSummitCarrierBridgeCompareAlignment_taggedGraph_*`**, **`nemsFe3IndexedPhantomOps_taggedGraph_align_factors_*`**).
  **Section-aware pack (Working queue item 3):** **`nemsSummitSectionAware_pack_isPullback_bridge_v5_forgetfulOnCarrier`** (+ variants); note **`SPEC_032_SA1`** (**tagged:** **`nemsSummitSectionAware_pack_taggedGraph_*`**, corpus **`nemsSummitSectionAware_pack_corpusLevel1Id_taggedGraph_*`**).
